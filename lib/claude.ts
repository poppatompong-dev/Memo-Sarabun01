...
export { AI_MODELS, DEFAULT_MODEL, type ModelKey } from '@/lib/ai-models'
...
{
  "content_background":"<เรื่องเดิม>",
  "content_facts":"<ข้อเท็จจริง>",
  "content_consideration":"<ข้อพิจารณา>",
  "changes":["<รายการสรุปสิ่งที่ AI ทำเป็นภาษาไทยกระชับ 3-6 ข้อ>"],
  "references":["<หลักอ้างอิงที่ใช้ เช่น 'ระเบียบสำนักนายกฯ ว่าด้วยงานสารบรรณ พ.ศ. ๒๕๒๖ ข้อ ๑๒'>"]
}

หมายเหตุ:
- ถ้าบริบทไม่มีข้อมูลพอสำหรับส่วนใด ส่งกลับเป็นสตริงว่าง ("") ห้ามคิดเอาเอง
- changes ให้เขียนเป็นภาษาไทยกระชับ บอกว่าทำอะไรไป เช่น "ขึ้นต้นเรื่องเดิมด้วย 'ตามที่' และลงท้ายด้วย 'นั้น'", "แปลงเลขอารบิกเป็นเลขไทยทั้งหมด"
- references ระบุระเบียบ/ข้อกฎหมายที่อ้างอิงในการร่าง`

// claude -p อ่าน prompt จาก stdin — หลีกเลี่ยงปัญหา shell escaping
function runClaude(prompt: string, timeoutMs: number, modelId?: string): Promise<string> {
...
บริบท/ความต้องการ: ${params.context}`
  ...
  const modelId = params.model ? AI_MODELS[params.model].id : undefined
  const stdout = await runClaude(fullPrompt, 120_000, modelId)
...
ตอบเป็น JSON อย่างเดียว ห้ามมีข้อความอื่นนอก JSON:
{
  ...
  "changes":["<รายการสรุปสิ่งที่แก้ไขเป็นภาษาไทยกระชับ 3-8 ข้อ ระบุก่อน→หลัง ถ้าทำได้>"],
  "references":["<ระเบียบ/ข้อ ที่อ้างอิงในการแก้>"]
}

หมายเหตุ:
- changes ให้บอกชัดว่าแก้อะไร เช่น "เปลี่ยน 'ได้ทำการ' เป็น 'ได้ดำเนินการ'", "เพิ่ม 'นั้น' ปิดวรรคเรื่องเดิม", "แปลงเลขอารบิก ๔ จุดเป็นเลขไทย"
- ถ้าไม่ได้แก้อะไรเลย ให้ changes เป็น [] (array ว่าง)`
...
ตอบเป็น JSON อย่างเดียว ห้ามมีข้อความอื่นนอก JSON:
{
  ...
  "changes":["<รายการสิ่งที่แก้กระชับ 1-4 ข้อ เช่น 'ขึ้นต้นด้วย ขออนุมัติ', 'ตัดคำซ้ำ จัดการบริหาร'>"],
  "references":["<ระเบียบที่อ้าง>"]
}`

export async function polishSubject(subject: string, model?: ModelKey): Promise<{ subject: string; changes?: string[]; references?: string[]; _meta: DraftResult['_meta'] }> {
  const fullPrompt = SUBJECT_SYSTEM_PROMPT + '\n\nหัวเรื่องที่ต้องการปรับปรุง:\n' + subject
  const modelId = model ? AI_MODELS[model].id : undefined
  const stdout = await runClaude(fullPrompt, 30_000, modelId)
  const cliData = JSON.parse(stdout)
  if (cliData.is_error) throw new Error(cliData.result ?? 'Claude CLI error')
  ...
  return {
    ...
    changes: Array.isArray(parsed.changes) ? parsed.changes : undefined,
